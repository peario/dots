...
-- Fuzzy finder
now(function()
  add({ source = "alexpasmantier/tv.nvim" })
    ...
    channels = {
      files = {
        ...
        handlers = {
          ...
          ---@diagnostic disable-next-line: unused-local
          ["<Esc>"] = function(entries, config)
            local pat = "/.cargo/bin/tv"
            local buffers = vim.api.nvim_list_bufs()

            for _, bufnr in ipairs(buffers) do
              if vim.api.nvim_buf_is_valid(bufnr) and vim.api.nvim_buf_get_name(bufnr):find(pat) then
                vim.cmd.bwipeout({ count = bufnr, bang = false })
              end
            end
          end,
        },
