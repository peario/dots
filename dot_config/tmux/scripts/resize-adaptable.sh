...
while getopts l:p:t: name; do
	...
	p)
		...
		;;
	t)
		# NOTE: just to hide warning about unexported/unused variable (tflag).
...
if [ "$layout_name" = "main-vertical" ]; then
	MAIN_PANE_SIZE=$(("$(tmux display -p '#{window_width}')" * "$percentage" / 100))
	MAIN_SIZE_OPTION='main-pane-width'

fi

if [ "$layout_name" = "main-horizontal" ]; then
	MAIN_PANE_SIZE=$(("$(tmux display -p '#{window_height}')" * "$percentage" / 100))
